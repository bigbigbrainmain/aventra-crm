...
export const handler = async (event) => {
    ...
    let name, email, proposal;
    try {
        ({ name, email, proposal } = JSON.parse(event.body || '{}'));
    } catch {
    ...
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto;">
            <div style="background: #DC2626; padding: 24px 32px;">
                <h1 style="color: white; margin: 0; font-size: 22px;">&#128226; Proposal Accessed</h1>
            </div>
            <div style="background: #f9f9f9; padding: 32px; border: 1px solid #e5e7eb;">
                <p style="font-size: 18px; font-weight: bold; color: #0F0F0F; margin-top: 0;">
                    A customer has opened the <strong>${siteName}</strong> proposal.
                </p>
                <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
                    ...
                    <tr>
                        <td style="padding: 10px 0; color: #6B7280; font-size: 14px;">Proposal</td>
    ...
    try {
        const res = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
                ...
                subject: `Proposal Accessed - ${siteName} (${name})`,
                html,
            }),
        });
